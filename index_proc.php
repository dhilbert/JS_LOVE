...
if($info==Null) {
	...
	$insert_sql	 = "
		insert save_crypotology set 
			count_cryptology ='".$count_cryptology."',
			reg_date ='".$reg_date."',
			reg_ip ='".$reg_ip."',
			count ='".$info['count']."'
	...
	echo "<script>
			alert('총 ".$info['count']."개의 sensor가 등록 되었습니다. ');
...
echo "<script>
			...
			parent.location.replace('/thebiz_api/');
		 </script> ";
...
if($info==Null) {
	...
	if($info['admin_pw']==MD5($admin_pw)){
			...
			echo "<script>
				alert('환영합니다.');
				parent.location.replace('/thebiz_api/home.php');
			</script> ";
